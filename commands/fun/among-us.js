...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    getPlayers.on('collect', async (button) => {
      ...
      button.reply({
        content: `You have joined the game, you are ${emojis[temp].toString()}`,
        ephemeral: true
      });
    });
    ...
    getPlayers.on('end', async () => {
      ...
      const impostor = gamedata[Math.floor(Math.random() * gamedata.length)];
      ...
      const collector = await message.channel.createMessageCollector({
        filter: (msg) => gamedata.some((v) => v.user.id === msg.author.id)
      });
      let emergencies = 3;
