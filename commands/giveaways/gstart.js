...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (!time)
      return message.reply(`Could not parse \`${args[0]}\` as time.${example}`);
    args.shift();
    ...
    // req prize --tags
    if (!args[0])
      return message.reply(`Please provide Requirements or "none"!${example}`);
    const rawq = args[0];
    let requirements = [];
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `Use the button to enter!\n**Time**: ${require('ms')(time, {
          long: true
        })} (<t:${((new Date().getTime() + parseInt(time)) / 1000).toFixed(
          0
    ...
    if (requirements && requirements.length) {
      embed.addFields([
        {
          name: 'Requirements:',
          value: `Roles: ${requirements.map((val) => `<@&${val}>`).join(', ')}`,
          inline: false
        }
      ]);
    }
    ...
    client.channels.cache.get('950386296436695061').send({
      embeds: [
        new EmbedBuilder()
          ...
          .addFields([{ name: 'Host', value: message.member.toString() }]),
        {
          title: 'Raw',
