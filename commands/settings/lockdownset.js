    ...
    if (todo === 'message') {
      ...
      if (!args[0]) {
        if (serversettings.lockdownSet.message) {
          ...
          return message.channel.send(
            `Please provide a message to display while the server is locked.`
          );
        }
      } else {
      ...
      if (!args[0]) {
        if (serversettings.lockdownSet.channels.includes(message.channel.id)) {
          message.channel.send(
            `This channel is already in the list of channels to be locked!`
          );
          return;
        } else {
        ...
        if (message.mentions.channels.size > 1) {
          ...
          if (serversettings.lockdownSet.channels.length) {
            serversettings.lockdownSet.channels =
              serversettings.lockdownSet.channels.concat(channels);
          } else {
          ...
          if (serversettings.lockdownSet.channels.includes(id)) {
            return message.channel.send(
              `<#${id}> was already there in the list.`
            );
          } else {
          ...
          return message.channel.send(
            `Please tag the channel, example: <#${message.channel.id}>`
          );
        }
      }
    } else if (todo === 'remove') {
      ...
      if (!args[0]) {
        if (serversettings.lockdownSet.channels.includes(message.channel.id)) {
          serversettings.lockdownSet.channels =
            serversettings.lockdownSet.channels.filter(
              (channel) => channel !== message.channel.id
            );
          serversettings.save();

          return message.channel.send(
            `Done! Removed <#${message.channel.id}> channel from the lockdown list`
          );
        } else {
          return message.channel.send(
            `Which channel do you want me to remove, please specify that.`
          );
        }
      } else {
        if (message.mentions.channels.size > 1) {
          ...
          if (serversettings.lockdownSet.channels.length) {
            for (i = 0; i < channels.length; i++) {
              serversettings.lockdownSet.channels =
                serversettings.lockdownSet.channels.filter(
                  (channel) =>
                    !serversettings.lockdownSet.channels.includes(channels[i])
                );
            }
          }
          serversettings.save();
      ...
      if (!serversettings.lockdownSet.channels)
        return message.channel.send(`No channels are added to the list`);
