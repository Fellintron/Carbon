...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!args[0])
      return message.channel.send(
        'Please specify which setting you would like to change.'
      );
    ...
    if (!guild) {
      ...
      await newGuild.save().catch((e) => {
        message.channel.send(
          `There was an error saving this new guild!\nERROR: \`${e}\``
        );
      });
    }
    ...
    if (setting === 'donorole') {
      ...
      if (!args[0])
        return message.channel.send(
          'Please specify one of these: `add`, `remove` or `clear`'
        );
      let todo = args[0].toLowerCase();
      if (!['add', 'remove', 'clear'].includes(args[0].toLowerCase()))
        return message.channel.send(
          'Please specify one of these: `add`, `remove` or `clear`'
        );
      ...
      if (todo === 'add') {
        ...
        if (!validRole)
          return message.channel.send(
            `ERROR: I could not find any role with the id \"${roleID}\"`
          );
        ...
        if (!guild.donationRoles.includes(roleID)) {
          return message.channel.send(
            'How can I remove something that does not exist, that role is not whitelisted for it to be removed.'
          );
        }
        let newData;
        if (Array.isArray(guild.donationRoles)) {
          newData = guild.donationRoles.filter(
            (d) => !guild.donationRoles.includes(d)
          );
        } else
          newData = guild.donationRoles.filter(
            (d) => d !== guild.donationRoles
          );
      ...
      if (!['true', 'fakse'].includes(yesno))
        return message.channel.send(`Please specify either true or false`);
