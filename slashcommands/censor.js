...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((cmd) => {
      return cmd.setName('list').setDescription('List of all the censors.');
    }),
  /**
  ...
  async execute(interaction, client) {
    if (!client.config.trustedAccess.includes(interaction.user.id)) return;
    ...
    if (command === 'add') {
      ...
      if (sensor) {
        ...
        return interaction.reply(`Removed the censor with ID \`${id}\`!`);
      } else {
        return interaction.reply(
      ...
      const sensors = server.censors.censors.map(
        (censor, index) =>
          `${index + 1}. Name: \`${censor.censor}\`(${censor.type})\nID: ${
            censor.id
          }`
      );
